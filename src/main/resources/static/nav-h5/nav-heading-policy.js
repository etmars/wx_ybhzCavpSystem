...
window.NavHeadingPolicy = {
  ...
  allowKnnHeadingCalibrate(flow, navRealForwardConfirmed, bumpHeadingAnchored, rotationOnlyLocked) {
    ...
    if (flow === this.NavFlow.PARKING_ENTRY || flow === this.NavFlow.FIND_CAR_WALK) return true;
  ...
  allowFirstKnnPathAlign(flow, pendingFirstKnn) {
    return pendingFirstKnn && (
      flow === this.NavFlow.PARKING_ENTRY
      || flow === this.NavFlow.PICKUP_EXIT
      || flow === this.NavFlow.FIND_CAR_WALK
    );
  },

  shouldSetPendingFirstKnnHeadingAlign(flow) {
    return flow === this.NavFlow.PARKING_ENTRY
      || flow === this.NavFlow.PICKUP_EXIT
      || flow === this.NavFlow.FIND_CAR_WALK;
  },
  ...
  useRouteHeadingForIcon(flow, phase) {
    return (flow === this.NavFlow.PARKING_ENTRY || flow === this.NavFlow.FIND_CAR_WALK)
      && phase === this.NavMotionPhase.NO_KNN;
  },
};
